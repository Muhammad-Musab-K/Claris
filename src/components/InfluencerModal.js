...
export default function InfluencerModal() {
    ...
    const ApprovedReq = async () => {
        await dispatch(HandleReq({ id: influencerData.id, res: "approved", token }));
        dispatch(isOpen(false));
    };
    const NotApprovedReq = () => {
        dispatch(HandleReject({ id: influencerData.id, res: "rejected", token }));
        dispatch(isOpen(false));
    };
    const handleButton = influencerData?.UserStatus
                                ...
                                <DialogPanel className="w-full max-w-sm rounded-xl flex flex-col gap-3 bg-white p-6 shadow">
                                    <div className='flex gap-4 relative'>
                                        <img
                                            ...
                                            alt="Profile pic"
                                        />
                                        <HiMiniXMark onClick={close} className='cursor-pointer absolute right-0 text-xl hover:opacity-45' />
                                        <div>
                                            <h6 className='text-black md:font-semibold text-lg'>{influencerData?.name}</h6>
                                            {influencerData?.IG_account && (

                                                <Link to={influencerData.IG_account.includes("https://www.instagram.com/")
                                                    ? influencerData.IG_account
                                                    : `https://www.instagram.com/${influencerData?.IG_account}`}>
                                                    <img className="h-8 w-8" src={Insta} alt="Instagram" />
                                                </Link>
                                            )}
                                            {influencerData?.Tiktok_account && (

                                                <Link to={influencerData.Tiktok_account.includes("https://www.tiktok.com/")
                                                    ? influencerData.Tiktok_account
                                                    : `https://www.tiktok.com/${influencerData.Tiktok_account}`}>
                                                    <img className="h-8 w-8" src={TikTok} alt="Instagram" />
                                                </Link>
                                            )}
                                        </div>
                                    </div>
